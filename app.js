...
client.on('message', async msg => {
    ...
    */
    ...
    if (command === "purge") {
        if (msg.member.roles.has(msg.guild.roles.find("name", "Staff").id)) { //Must be staff to purge
            if (!arguments[0]) return; //If no arguments, return.
            const deleteNum = parseInt(arguments[0]);
            if (!deleteNum) return; //make sure we have a number
            ...
            msg.channel.bulkDelete(fetched)
                .then(() => {
                    logMessage(msg, {
                        command: `${command.charAt(0).toUpperCase()}${command.slice(1)}`,
                        info: arguments[1] ? arguments.splice(1).toString().replace(/,/g, ` `) : `${deleteNum} message(s) have been deleted.`
                        //Extra info is either a reason for purge or number of messages deleted
                    })
                })
                .catch((err) => {
                    //Note = error message
                    if (err == "DiscordAPIError: You can only bulk delete messages that are under 14 days old.") {
                        msg.reply(`Can't delete messages that are over 14 days old.`)
                        logMessage(msg, {
                            command: `${command.charAt(0).toUpperCase()}${command.slice(1)}`,
                            info: "Error deleting messages: Messages over 14 days old may not be deleted using this command."
                        })
                    } else {
                        msg.reply(`Couldn't delete messages because of: ${(err).toString().replace("DiscordAPIError:", "")}`)
                        logMessage(msg, {
                            command: `${command.charAt(0).toUpperCase()}${command.slice(1)}`,
                            info: `Failed to delete messages: ${(err).toString().replace("DiscordAPIError:", "")}`
                        })
                    }
                })
        } else {
            //Mute a user if they try to purge and are not staff, uncomment if this is decided
            /*
                ...
                logMessage(msg, {
                    ...
                    info: `Automatic mute of a non-staff member by the system for abuse of ?purge command.`
                })
            */
        }
    }
...
const logMessage = function (message, obj) {
    1
